...
- 发布 62 个种子节点组成的 `site/data/catalog.json`。
- 发布 61 个来源入口组成的 `site/data/sources.json`，标明第一方/线索源、接入方式与使用边界。
- 发布 19 个采集任务组成的 `site/data/ingestion-jobs.json`。
- 发布 46 个结构化入口组成的 `site/data/field-guides.json`，连接知识节点与一手来源。
- 已发布 16 篇知识正文，覆盖机器人基础、硬件选型与跨领域实验设计；每篇标明来源、关联节点、编辑状态和是否经过人工复核。
- 首批硬件价格快照以追加式 JSON 保存，价格与地区、币种、税运费、库存和日期绑定。
- 首批论文身份以 `content/work-identities.json` 保存；未知 DOI、OpenReview、代码或 venue 显式保留为 `null`。
