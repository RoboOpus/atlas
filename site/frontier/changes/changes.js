...
try {
  ...
  let limit = 20;
  document.querySelector("#changes-status").textContent = `历史归档 ${papers.size} 篇 · 雷达展示 ${windowData.papers.length} 篇 · ${events.length} 批记录 · ${archive.last_success_at ? `最近成功收录 ${archive.last_success_at}` : "归档已初始化，等待新版采集器首次成功运行"}`;
  function render() {
    const grid = document.querySelector("#changes"); grid.replaceChildren();
    for (const event of events.slice(0, limit)) {
      const card = make("article", "", "search-result");
      card.append(make("p", event.at, "result-meta"), make("h2", event.kind === "baseline" ? `已有数据基线 · ${event.baseline_ids.length} 篇` : `新收录 ${event.added.length} 篇 · 元数据更新 ${event.updated.length} 篇`));
      ...
      for (const [label, ids] of [["基线", event.baseline_ids ?? []], ["新收录", event.added], ["元数据更新", event.updated]]) {
